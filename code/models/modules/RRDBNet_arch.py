...
class RRDBNet(nn.Module):
    ...
    def forward(self, x, get_steps=False):
        ...
        fea_up2 = self.upconv1(F.interpolate(last_lr_fea, scale_factor=0.5, mode='bilinear', align_corners=False, recompute_scale_factor=True))
        fea = self.lrelu(fea_up2)

        fea_up4 = self.upconv2(F.interpolate(fea, scale_factor=0.5, mode='bilinear', align_corners=False, recompute_scale_factor=True))
        ...
        out = self.conv_last(self.lrelu(self.HRconv(last_lr_fea)))
        ...
        if fea_up0_en:
            results['fea_up0'] = F.interpolate(fea_up4, scale_factor=1/2, mode='bilinear', align_corners=False, recompute_scale_factor=True)
