...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        self.H = H
        self.W = W
        self.scaleH = opt_get(opt, ['datasets', 'train', 'GT_size']) / H
        self.scaleW = opt_get(opt, ['datasets', 'train', 'GT_size']) / W
    ...
    def arch_FlowStep(self, H, K, LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling, flow_permutation,
        ...
        for k in range(K):
            position_name = get_position_name(H, self.opt)
    ...
    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None):
        ...
        GT_size = opt_get(self.opt, ['datasets', 'train', 'GT_size'])
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            size = shape[2]
            level = int(np.log(GT_size / size) / np.log(2))
...
def get_position_name(H, opt):
    downscale_factor = opt['datasets']['train']['GT_size'] // H
    position_name = 'fea_up{}'.format(opt['scale'] / downscale_factor)
