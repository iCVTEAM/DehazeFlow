...
class DehazeFlowModel(BaseModel):
    def __init__(self, opt, step):
        super(DehazeFlowModel, self).__init__(opt)
        self.opt = opt

        self.heats = opt['val']['heats']
        self.n_sample = opt['val']['n_sample']
        self.hr_size = opt_get(opt, ['datasets', 'train', 'center_crop_hr_size'])
        self.hr_size = opt_get(opt, ['datasets', 'train', 'GT_size']) if self.hr_size is None else self.hr_size
        self.lr_size = self.hr_size
        ...
        self.netG = networks.define_Flow(opt, step)
        if opt['dist']:
            local_rank = torch.distributed.get_rank()
            torch.cuda.set_device(local_rank)
            device = torch.device("cuda", local_rank)
            self.netG = self.netG.to(device)
            self.netG = DistributedDataParallel(self.netG, device_ids=[local_rank],output_device=local_rank,find_unused_parameters=True)
            print('Dist Prepared '+ str(local_rank))
        else:
            self.netG = DataParallel(self.netG, device_ids=[Id for Id in range(torch.cuda.device_count())])
        # print network
        self.print_network()
    ...
    def optimize_parameters(self, step):
        ...
        with torch.cuda.amp.autocast():
            ...
            if weight_fl > 0:
                z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, reverse=False, add_gt_noise=True)
                nll_loss = torch.mean(nll)
                losses['nll_loss'] = nll_loss * weight_fl

            weight_l1 = opt_get(self.opt, ['train', 'weight_l1']) or 0
            weight_l2 = opt_get(self.opt, ['train', 'weight_l2']) or 0
            if weight_l1 > 0 or weight_l2 > 0:
                ...
                l2_loss = self.criterion(sr,self.real_H)
                losses['l1_loss'] = l1_loss * weight_l1
                losses['l2_loss'] = l2_loss * weight_l2
    ...
    def get_z(self, heat, seed=None, batch_size=1, lr_shape=None):
        ...
        if opt_get(self.opt, ['network_G', 'flow', 'split', 'enable']):
            ...
            H = int(lr_shape[2] // self.netG.module.flowUpsamplerNet.scaleH)
            W = int(lr_shape[3] // self.netG.module.flowUpsamplerNet.scaleW)
            ...
            z_size = int(self.lr_size // 4 // (2 ** (L - 3)))
    ...
    def load(self):
        ...
        if get_resume_model_path is not None:
            print('rank'+str(self.opt['rank']) + 'load' + get_resume_model_path)
            self.load_network(get_resume_model_path, self.netG, strict=True, submodule=None)
            return

        load_path_G = self.opt['path']['pretrain_model_G']
        if load_path_G is not None:
            if self.opt['path']['G_complete']:
                logger.info('Loading model for complete model [{:s}] ...'.format(load_path_G))
                self.load_network(load_path_G, self.netG, strict=True, submodule=None)
            else:
